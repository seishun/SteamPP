...
namespace Steam
{
	...
	{
	public:
		SteamClient(std::function<void(const std::string& host, std::uint16_t port)> connect,
	...
	private:
		std::function<void(const std::string& host, std::uint16_t port)> connect_;
		std::function<void(std::size_t length, std::function<void(unsigned char* buffer)> fill)> write_;
		std::function<void(std::function<void()> callback, int timeout)> set_interval_;
		...
		void WritePacket(size_t length, const std::function<void(unsigned char* buffer)> &fill);
	};
